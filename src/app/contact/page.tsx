...
export default function Contact() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-extrabold">📞 Contact Us – Planet Interio</h1>
      <div className="mt-3 space-y-1 text-sm">
        <p>📱 Call: <a className="underline font-bold" href="tel:+919429691445">{SITE.phone}</a></p>
        <p>💬 WhatsApp: <a className="underline font-bold" href="https://wa.me/918974499282">{SITE.whatsapp}</a></p>
        <p>🏠 Store: {SITE.address}</p>
        <p>🏠 Workshop: {SITE.workshop}</p>
        <p className="text-gray-500">{SITE.hours}</p>
      </div>
      <div className="grid md:grid-cols-2 gap-4 mt-6">
        <form className="bg-white border rounded-2xl p-4 space-y-2 text-sm" onSubmit={(e) => { e.preventDefault(); setOk(true); }}>
          ...
          <select className="border rounded-lg px-3 py-2.5 w-full" aria-label="Topic"><option>Delivery / Click & Collect</option><option>Cancellation / Returns / Warranty</option><option>Online payment help</option><option>Customised furniture / onsite installation</option><option>Product question</option></select>
          <textarea required placeholder="How can we help?" className="border rounded-lg px-3 py-2.5 w-full" aria-label="Message" />
          <button className="bg-[#D21F26] text-white font-extrabold rounded-full px-6 py-2.5">Send</button>
          {ok && <p role="status" className="text-green-700">Thanks — we reply within one working day. For faster help, WhatsApp us at {SITE.whatsapp}.</p>}
        </form>
        <div className="text-sm space-y-4" id="faq">
          <div className="bg-white border rounded-2xl p-4"><h2 className="font-extrabold">Delivery / Click & Collect</h2><p className="text-gray-600 mt-1">Imphal Core (795001–795004): same/next-day. Greater Imphal: 1–2 days. Manipur: 2–5 days. Or collect free from our Canchipur store with SMS confirmation.</p></div>
