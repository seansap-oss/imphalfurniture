...
export async function POST(req: Request) {
  ...
  const fee = b.mode === "pickup" ? 0 : deliveryFee(b.pin, sub);
  const orders = await readJSON<any[]>("orders.json", []);
  const id = `IF-${Date.now().toString().slice(-8)}`;
  const order = { id, date: new Date().toISOString(), status: "Paid", pay: b.pay, mode: b.mode, name: b.name, phone: b.phone, email: b.email, pin: b.pin, city: b.city, items: b.lines, subtotal: sub, delivery: fee, total: sub + fee, history: [{ s: "Paid", t: new Date().toISOString() }] };
