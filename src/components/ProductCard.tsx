    ...
    <article className="bg-white rounded-2xl overflow-hidden border hover:shadow-lg transition group flex flex-col">
      <div className="relative">
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          {p.badges.slice(0, 2).map((b) => (
            <span key={b} className={`text-[10px] font-extrabold px-2 py-1 rounded ${b === "SALE" ? "bg-red-600 text-white" : b === "NEW" ? "bg-black text-[#FFD400]" : "bg-[#FFD400] text-black"}`}>{b}</span>
          ))}
        </div>
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        ...
        <div className="flex gap-2 mt-2">
          <button onClick={() => cart.add({ slug: p.slug, qty: 1, price: p.price, title: p.name, image: p.images[0] })} className="flex-1 bg-[#FFD400] font-extrabold text-sm rounded-full py-2.5 min-h-[44px] hover:bg-yellow-300">Add to Cart</button>
